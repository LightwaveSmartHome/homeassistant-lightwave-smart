    ...
    for device_id in link.get_device_ids():
        try:
            if link.devices[device_id].is_gen2():
                fws.append(LWRF2Update(name, device_id, link, homekit, FIRMWARE_DESCRIPTION))
...
class LWRF2Update(UpdateEntity):
    ...
    def __init__(self, name, device_id, link, homekit, entity_description):
        # _LOGGER.warning("UPDATE ###########################################################################")
        
        _LOGGER.debug("Adding Firmware update: %s - %s ", name, device_id)
        ...
        self.feature_set = self._lwlink.featuresets[self._device_id]
        self.installed_version = self.feature_set.firmware_version
        
        # TODO - get latest version from WS API
        
        # _LOGGER.warning("UPDATE #####@ %s", dir(feature_set))
        
        self.latest_version = self.feature_set.latest_firmware_version
        self.release_summary = self.feature_set.latest_firmware_release_summary
    ...
    def async_update_callback(self, **kwargs):
        ...
        try:
            ...
            _LOGGER.warning(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> async_update_callback - Update update: %s - %s - %s", self.entity_id, kwargs, self.entity_description.key)
            self.latest_version = self.feature_set.latest_firmware_version
            self.release_summary = self.feature_set.latest_firmware_release_summary
